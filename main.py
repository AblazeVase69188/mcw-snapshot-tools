...
if version_type not in ["Release", "N/A"]:
    disambig_page_url = WIKI_BASE_URL + f"{parent.split('.')[0]}.x"
    print(f"版本号消歧义页面：{disambig_page_url}?action=edit")
...
if version_type == "Release":
    print(f"计划版本页面：{WIKI_BASE_URL}%E8%AE%A1%E5%88%92%E7%89%88%E6%9C%AC?action=edit")
    print("将段落内容替换为“Java版暂无已知的计划版本。”")
    
    print(f"版本记录页面：{WIKI_BASE_URL}Java%E7%89%88%E7%89%88%E6%9C%AC%E8%AE%B0%E5%BD%95?action=edit")
    print(f"在表格第一行添加更新名称，表格最后一行改为{release_dt_date}")

    print(f"指南页面：{WIKI_BASE_URL}Java%E7%89%88%E6%8C%87%E5%8D%97?action=edit")
...
if is_initial_snapshot:
    ...
    parent_version_page_section_0 += """}}</ref>"""
    print(f"正式版页面：{WIKI_BASE_URL}Java%E7%89%88{parent}?action=edit")
    print("内容为：")
    print("----")
